      ...
      <v-container>
        ...
        <v-row class="py-4">
          ...
          <v-col cols="12" sm="4" md="3" lg="2">
            <v-btn color="info" prepend-icon="mdi-plus" variant="flat" block :to="{ name: 'IdiomasForm', params: { id: '0' } }">
              Nuevo
            </v-btn>
          </v-col>
        </v-row>
        <v-table fixed-header hover density="comfortable">
          <thead class="grey--text text--darken-2">
            <tr>
              ...
              <th width="40%" class="text-center bg-secondary" style="border: 1px solid white; border-width: 0 1px 0 0;">
                Nombre
              </th>
              <th width="15%" class="text-center bg-secondary" style="border: 1px solid white; border-width: 0 1px 0 0;">
          ...
          <tbody v-if="paginacion.total > 0">
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="text-center">{{ item.resolucionMinisterial || '-' }}</td>
              <td class="text-center">
                <v-img
                  ...
                  height="30"
                  max-height="30"
                  :src="`${url}/uploads/idiomas/${item.imagen}`"
                ></v-img>
                <div v-else>-</div>
              </td>
              <td class="text-center">
                <v-btn density="compact" color="success" :to="{ name: 'NivelesIndex', params: { id: item.id } }">
                  Niveles
                </v-btn>
              </td>
              <td class="text-center">
                ...
                ></v-btn>
                <v-btn
                  :to="{ name: 'IdiomasForm', params: { id: item.id } }"
                  size="x-small"
                  icon="mdi-pencil"
                  color="warning"
                  class="mx-1"
...
import ModalDetalle from '@/components/idiomas/Show.vue'
