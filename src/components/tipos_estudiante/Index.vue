      ...
      <v-container>
        ...
        <v-row class="py-4">
          ...
          <v-col cols="12" sm="4" md="3" lg="2">
            <v-btn color="info" prepend-icon="mdi-plus" variant="flat" block :to="{ name: 'TiposEstudianteForm', params: { id: '0' } }">
              Nuevo
            </v-btn>
          </v-col>
        </v-row>
        <v-table fixed-header hover density="comfortable">
          <thead class="grey--text text--darken-2">
            <tr>
              <th width="60%" class="text-center bg-secondary" style="border: 1px solid white; border-width: 0 1px 0 0;">
                Nombre
              </th>
          ...
          <tbody v-if="paginacion.total > 0">
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="text-center">{{ item.militar ? 'SI' : ' NO' }}</td>
              ...
              <td class="text-center">
                ...
                ></v-btn>
                <v-btn
                  :to="{ name: 'TiposEstudianteForm', params: { id: item.id } }"
                  size="x-small"
                  icon="mdi-pencil"
                  color="warning"
                  class="mx-1"
...
import ModalDetalle from '@/components/tipos_estudiante/Show.vue'
...
const breadcrumbs = [
  {
    text: 'Tipos de Estudiante',
    to: { name: 'TiposEstudianteIndex' },
    disabled: false,
  },
]
const detalle = ref()
...
const modalEliminar = reactive({
  id: null,
  url: 'tipos_estudiante',
  titulo: 'el tipo de estudiante',
  codigo: null,
  abrir: false,
})
