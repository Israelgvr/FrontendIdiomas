    ...
    <v-card>
      <v-card-title class="bg-indigo">
        <v-row justify="space-between" aligh="center" align-content="center" dense>
          <v-col cols="11">Detalle de la Filial</v-col>
      ...
      <v-card-text>
        <v-table density="compact">
          ...
          <tbody>
            ...
            <tr>
              <th class="text-right pr-2">Departamento:</th>
              <td class="font-weight-bold">{{ registro.ciudad.nombre }}</td>
            </tr>
            <tr>
              <th class="text-right pr-2">Localidad:</th>
              <td class="font-weight-bold">{{ registro.localidad }}</td>
            </tr>
            <tr>
              <th class="text-right pr-2">Direccion:</th>
              <td class="font-weight-bold">{{ registro.direccion }}</td>
            </tr>
            <tr>
...
const getDatos = async function(id) {
  try {
    cargando.value = true
    const response = await axios.get(`/filiales/${id}`)
