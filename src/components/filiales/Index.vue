    ...
    <v-sheet>
      <v-container>
        ...
        <v-row class="py-4">
          ...
          <v-col cols="12" sm="4" md="3" lg="2">
            <v-btn color="info" prepend-icon="mdi-plus" variant="flat" block :to="{ name: 'FilialesForm', params: { id: '0' } }">
              Nuevo
            </v-btn>
          </v-col>
        </v-row>
        <v-table fixed-header hover density="comfortable">
          ...
          <tbody v-if="paginacion.total > 0">
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="text-center">{{ item.ciudad.nombre }}</td>
              ...
              <td class="text-center">
                ...
                ></v-btn>
                <v-btn
                  :to="{ name: 'FilialesForm', params: { id: item.id } }"
                  size="x-small"
                  icon="mdi-pencil"
                  color="warning"
                  class="mx-1"
...
import ModalDetalle from '@/components/filiales/Show.vue'
...
const descargarExcel = async function() {
  try {
    cargando.value = true
    const response = await axios.get('excel/filiales', {
      responseType: 'blob',
    })
    ...
    link.setAttribute('download', `filiales.xlsx`)
