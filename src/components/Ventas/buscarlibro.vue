      ...
      <v-table fixed-header hover dense>
        ...
        <tr  v-for="(item, index) in paginatedItems" :key="index">
          ...
          <td class="text-center">
            <v-btn @click="agregarACompra(item)" color="primary">
              Agregar
            </v-btn>
          </td>
...
export default {
  ...
  methods: {
    fetchData() {
      axios.get('http://127.0.0.1:1659/listar')
        .then(response => {
    ...
    agregarACompra(item) {
      // Emitir evento con los datos del libro seleccionado
      this.$emit('libro-agregado', item);
    }
  },
  computed: {
    paginatedItems() {
