...
const breadcrumbs = [
  {
    ...
    to: { name: 'CiudadesForm', params: { id: route.params.id } },
    disabled: false,
  },
]
...
const enviarFormulario = async function() {
  try {
    ...
    if (form.value.id !== null) {
      response = await axios.put(`/ciudades/${form.value.id}`, form.value)
    } else {
      response = await axios.post('/ciudades', form.value)
    }
    alerta(response.data.message)
    router.push({ name: 'CiudadesIndex' })
  } catch(error) {
...
const getDatos = async function() {
  try {
    ...
    if (route.params.id !== '0') {
      const response = await axios.get(`/ciudades/${route.params.id}`)
