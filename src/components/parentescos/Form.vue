...
const breadcrumbs = [
  {
    ...
    to: { name: 'ParentescosForm', params: { id: route.params.id } },
    disabled: false,
  },
]
...
const enviarFormulario = async function() {
  try {
    ...
    if (form.value.id !== null) {
      response = await axios.put(`/parentescos/${form.value.id}`, form.value)
    } else {
      response = await axios.post('/parentescos', form.value)
    }
    alerta(response.data.message)
    router.push({ name: 'ParentescosIndex' })
  } catch(error) {
...
const getDatos = async function() {
  try {
    ...
    if (route.params.id !== '0') {
      const response = await axios.get(`/parentescos/${route.params.id}`)
