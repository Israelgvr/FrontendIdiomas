    ...
    <v-sheet>
      <v-container>
        ...
        <v-row class="py-4">
          ...
          <v-col cols="12" sm="3" lg="2">
            <v-btn color="info" prepend-icon="mdi-plus" variant="flat" block :to="{ name: 'CursosForm', params: { id: '0' } }">
              Nuevo
            </v-btn>
          </v-col>
        </v-row>
        <v-table fixed-header hover density="comfortable">
          ...
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="text-center">{{ $filters.hora(item.horaInicial) }} - {{ $filters.hora(item.horaFinal) }}</td>
            <td class="text-center">
              <v-chip :color="item.activo ? 'info' : 'success'">
                {{ item.activo ? 'EN CURSO' : 'FINALIZADO' }}
              </v-chip>
            </td>
            <td class="text-center">
              <v-btn density="compact" color="success" :to="{ name: 'EstudiantesDemo', params: { id: item.id } }">
                Ver Estudiantes
              </v-btn>
            </td>
          </tr>
...
import ModalDetalle from '@/components/cursos/Show.vue'
...
const descargarExcel = async function() {
  try {
    cargando.value = true
    const response = await axios.get('excel/cursos', {
      params: {
        filialId: filial.value.id,
      },
      responseType: 'blob',
