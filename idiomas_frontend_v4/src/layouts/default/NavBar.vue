<template>
  ...
  >
    ...
    <v-list density="compact" nav>
      ...
      <v-list-item v-show="permiso('CURSOS')" prepend-icon="mdi-table-chair" title="Dashboard" :to="{ name: 'dashboard' }"></v-list-item>
      ...
      <v-list-group v-show="permiso('REPORTES')">
        ...
        <v-list-item :style="{ paddingLeft: paddingLeft }" prepend-icon="mdi-file-chart-outline" title="Ingresar" :to="{ name: 'CursoForm'}"></v-list-item>
      </v-list-group>
      <v-list-group v-show="permiso('REPORTES')">
        <template v-slot:activator="{ props }">
