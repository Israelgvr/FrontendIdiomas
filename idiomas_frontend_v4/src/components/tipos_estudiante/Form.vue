                ...
                <v-card-text>
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" md="2" class="pb-0">
                      <v-select
                        density="compact"
                        v-model="form.militar"
                        label="Activo"
                        variant="outlined"
                        :error="errores.militar.length > 0"
                        :error-messages="errores.militar.length ? errores.militar[0] : ''"
                        :items="selectorSiNo"
                        item-title="codigo"
                        item-value="id"
                      ></v-select>
...
const breadcrumbs = [
  {
    ...
    to: { name: 'TiposEstudianteForm', params: { id: route.params.id } },
    disabled: false,
  },
]
...
const enviarFormulario = async function() {
  try {
    ...
    if (form.value.id !== null) {
      response = await axios.put(`/tipos_estudiante/${form.value.id}`, {
        ...form.value,
        filialId: filial.value.id,
      })
    } else {
      response = await axios.post('/tipos_estudiante', {
        ...form.value,
        filialId: filial.value.id,
      })
    }
    alerta(response.data.message)
    router.push({ name: 'TiposEstudianteIndex' })
  } catch(error) {
...
const getDatos = async function() {
  try {
    ...
    if (route.params.id !== '0') {
      const response = await axios.get(`/tipos_estudiante/${route.params.id}`)
