...
const breadcrumbs = [
  {
    ...
    to: { name: 'ModulosForm', params: { id: route.params.id } },
    disabled: false,
  },
]
...
const enviarFormulario = async function() {
  try {
    ...
    if (form.value.id !== null) {
      response = await axios.put(`/modulos/${form.value.id}`, form.value)
    } else {
      response = await axios.post('/modulos', form.value)
    }
    alerta(response.data.message)
    router.push({ name: 'ModulosIndex' })
  } catch(error) {
...
const getDatos = async function() {
  try {
    ...
    if (route.params.id !== '0') {
      const response = await axios.get(`/modulos/${route.params.id}`)
