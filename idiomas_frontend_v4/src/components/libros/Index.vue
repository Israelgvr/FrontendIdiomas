    ...
    <v-sheet>
      <v-container>
        ...
        <v-row class="py-4">
          ...
          <v-col cols="12" sm="3" lg="2">
            <v-btn color="info" prepend-icon="mdi-plus" variant="flat" block :to="{ name: 'LibrosForm', params: { id: '0' } }">
              Nuevo
            </v-btn>
          </v-col>
        </v-row>
            ...
            <tr v-for="item in items" :key="item.id">
              ...
              <td class="text-center">
                <v-img
                  ...
                  height="40"
                  max-height="40"
                  :src="`${url}/uploads/libros/${item.imagen}`"
                ></v-img>
                <div v-else>-</div>
              </td>
              <td class="text-center">
                ...
                ></v-btn>
                <v-btn
                  :to="{ name: 'LibrosForm', params: { id: item.id } }"
                  size="x-small"
                  icon="mdi-pencil"
                  color="warning"
                  class="mx-1"
...
import ModalDetalle from '@/components/libros/Show.vue'
...
const descargarExcel = async function() {
  try {
    cargando.value = true
    const response = await axios.get('excel/libros', {
      params: {
        filialId: filial.value.id,
      },
      responseType: 'blob',
