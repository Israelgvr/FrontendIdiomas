    ...
    <v-card>
      <v-card-title class="bg-indigo">
        <v-row justify="space-between" aligh="center" align-content="center" dense>
          <v-col cols="11">Detalle del Idioma</v-col>
      ...
      <v-card-text>
        <v-table density="compact">
          ...
          <tbody>
            ...
            <tr>
              <th class="text-right pr-2">Descripción:</th>
              <td class="font-weight-bold">{{ registro.descripcion }}</td>
            </tr>
            <tr>
              <th class="text-right pr-2">Resolucion Ministerial:</th>
              <td class="font-weight-bold">{{ registro.resolucionMinisterial }}</td>
            </tr>
            <tr>
              ...
              <td class="font-weight-bold">
                <v-img
                  ...
                  height="150"
                  max-height="150"
                  :src="`${url}/uploads/idiomas/${registro.imagen}`"
                ></v-img>
                <div v-else>-</div>
              </td>
            </tr>
...
const getDatos = async function(id) {
  try {
    cargando.value = true
    const response = await axios.get(`/idiomas/${id}`)
